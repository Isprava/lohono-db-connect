...
**Query Process:**
<<<<<<< HEAD
1. **Always try run_predefined_query first.** For ANY data question — scorecards, funnels, collection summaries, orderbooks, repeat guests, LTV, conversions, weekly insights, or any report-like request — ALWAYS call run_predefined_query first. Pass the user's question (or a close search term derived from it) as the search_term. The tool uses fuzzy matching against 60+ predefined queries and will find a match if one exists (40% confidence threshold). Do NOT skip this step. Do NOT try to guess whether a predefined query exists — just call the tool and let it search.
